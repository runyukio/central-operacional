...
import { emptySpaceMetric, finishSpaceMetric, spaceLobFamily, type MetricAccumulator } from "@/lib/meu-espaco-metrics";
...
export async function getSpaceResults(scope: MeuEspacoScope, query: URLSearchParams): Promise<SpaceResults> {
  ...
  if (!ids.length) return { period, groups: [], partners: [] };
  ...
  function targets(employeeId: string, date: string, updatedAt: Date | null, source: "production" | "quality" | "schedule") {
    ...
    return [employee.metric, group.metric, group.daily.get(date)!];
  }
  ...
  return { period, groups: [...groups.entries()].map(([lob, group]) => ({ lob, teamSize: group.employees, metric: finishSpaceMetric(group.metric, lob),
