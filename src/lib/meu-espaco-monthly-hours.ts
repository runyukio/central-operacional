...
export function summarizePartnerMonth(partner: {id:string; fullName:string; wbLogin:string; lob:{name:string}}, period: SpacePeriod, today: string, records: Record[], schedules: Schedule[], captured: Map<string, number>): SpaceMonthlyHoursRow {
  const row: SpaceMonthlyHoursRow = { id:partner.id,employeeId:partner.id,employeeName:partner.fullName,wbLogin:partner.wbLogin,lob:partner.lob.name,month:period.startDate.slice(0,7),
    plannedHours:0,actualHours:0,capturedHours:0,effectiveHours:0,adjustedHours:0,differenceMinutes:0,status:"Sem dados",realizedRecords:0,futureHours:0,projectedHours:null,missingPastSlots:0 };
  const days = new Set<string>(); let divergent=0, pending=0, noSchedule=0, futureSlots=0;
  for (const r of records) {
    const day=r.date.toISOString().slice(0,10);
    if(day<period.startDate||day>period.endDate||day>today) continue;
    row.realizedRecords++; days.add(day); row.actualHours+=r.actualHours;row.adjustedHours+=r.adjustedHours??0;
  ...
  row.projectedHours=row.realizedRecords||futureSlots?row.effectiveHours+row.futureHours:null;
  row.status=pending?`${pending} ajustes pendentes`:row.missingPastSlots?`${row.missingPastSlots} dias sem horas`:noSchedule?`${noSchedule} registros sem escala válida`:divergent?`${divergent} dias com divergência`:row.realizedRecords?"Sem divergência nas horas registradas":futureSlots?"Somente projeção":"Sem dados";
  return row;
}
