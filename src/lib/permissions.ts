...
export function isRtaSkill(value?: string | null) {
  ...
  return /(^|[^a-z0-9])rta([^a-z0-9]|$)/.test(normalized) || compact === "rta" || compact === "realtime" || compact.includes("realtimeanalyst") || compact.includes("analistarealtime") || compact.includes("analistarta");
}
...
export function isPocSkill(value?: string | null) {
  ...
  if (!normalized) return false;
  const compact = normalized.replace(/[^a-z0-9]/g, "");
  return /(^|[^a-z0-9])poc([^a-z0-9]|$)/.test(normalized) || compact === "poc" || compact.includes("pointofcontact");
}
...
export function canApproveRequest(user: PermissionUser, request?: { area?: string | null; type?: string | null }) {
  ...
  if (role === "SUPERVISOR") return Boolean(request && /(folga|day\s*off|troca|turno|escala)/i.test(`${request.area ?? ""} ${request.type ?? ""}`));
  return false;
}
...
export function canViewTeam(user: PermissionUser, employee?: PermissionEmployee) {
  ...
  return roleHasCapability(user.role, "CENTRAL") || roleHasCapability(user.role, "REALTIME_FULL") || roleHasCapability(user.role, "SCHEDULE_VIEW") || roleHasCapability(user.role, "EMPLOYEE_MAP");
}
