...
test("builds the ADS online productivity ranking with hourly and shift metrics", () => {
  ...
  assert.deepEqual(report.rows.map((row) => row.shiftModerationMs), [126 * 84_000, 104 * 90_000, 83 * 92_000]);
...
test("keeps the operational shift total when the China-day counter resets at 13h", () => {
  ...
  assert.equal(report.rows[0].shiftModerationMs, 396 * 50_000);
});
...
test("keeps the night shift total across Sao Paulo midnight", () => {
  ...
  assert.equal(report.rows[0].shiftModerationMs, 130 * 60_000);
...
test("excludes agents with zero submit from the report and averages", () => {
  ...
  assert.equal(report.rows[0].shiftModerationMs, 50 * 60_000);
...
test("empty ADS report has no agent moderation rows", () => {
...
test("keeps each agent's recorded moderation at full precision, not rounded AHT times submits", () => {
  const report = buildAdsOnlineProductivityReportSnapshot({
  ...
  assert.deepEqual(report.rows.map((row) => row.shiftModerationMs), [20_001, 20_002]);
