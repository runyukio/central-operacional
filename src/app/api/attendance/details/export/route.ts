...
import { exportAttendanceDetailXlsxData } from "@/lib/schedule-service";
import { buildXlsxResponse } from "@/lib/xlsx-export";

export async function GET(request: Request) {
  const actor = await getApiActor();
  const url = new URL(request.url);
  const result = await exportAttendanceDetailXlsxData(actor, {
  ...
  if ("error" in result) {
    ...
    return NextResponse.json({ error: failure.error, message: failure.error }, { status: failure.status ?? 400 });
  }
