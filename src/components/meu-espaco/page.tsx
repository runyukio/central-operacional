...
export function MeuEspacoPage() {
  ...
  return <div className={`${styles.root} space-y-5`}>
    <header className={styles.hero}>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2"><span className={styles.badge}><UsersRound className="h-3.5 w-3.5" />{currentName ? `Time de ${currentName}` : summary.data?.actor.broad ? "Todos os supervisores" : "Meu time"}</span><span className="text-xs text-muted">{dateLabel(period.startDate)} a {dateLabel(period.endDate)}</span>{summary.data && !summary.data.actor.canRespond ? <span className={styles.badge}>Somente consulta</span> : null}</div>
    </header>
