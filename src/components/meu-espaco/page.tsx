...
export function MeuEspacoPage() {
  ...
  const [resultsView, setResultsView] = useState("dashboard");
  ...
  function selectSupervisor(id: string) { if (!supervisorId && summary.data) setDirectory(summary.data.supervisors); setSupervisorId(id); setResultsView("dashboard"); }
  ...
  return <div className={`${styles.root} space-y-5`}>
    <header className={styles.hero}>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2"><span className={styles.badge}><UsersRound className="h-3.5 w-3.5" />{currentName ? `Time de ${currentName}` : summary.data?.actor.broad ? "Todos os supervisores" : "Meu time"}</span><span className="text-xs text-muted">{tab === "hours" ? "Horas consolidadas por mês" : tab === "pending" ? "Pendências e cobertura" : resultsView === "glide" && tab === "results" ? "Glide path mensal" : `${dateLabel(period.startDate)} a ${dateLabel(period.endDate)}`}</span>{summary.data && !summary.data.actor.canRespond ? <span className={styles.badge}>Somente consulta</span> : null}</div>
    </header>
    ...
    {tab === "overview" || (tab === "results" && resultsView === "dashboard") ? <SpacePeriodSlicer value={period} today={initialPeriod().endDate} onChange={setPeriod} /> : null}
    {!valid ? <p role="alert" className="text-red-700">Selecione um período válido para os indicadores.</p> : null}
    <fieldset><legend className="sr-only">Visão</legend><div className={styles.views}>{views.map(({ id, label, icon: Icon }) => <button key={id} type="button" aria-pressed={tab === id} onClick={() => { if (id === "pending") setPendingKind("all"); setTab(id); setResultsView("dashboard"); }} className={`${styles.chip} ${styles.view}`}><Icon aria-hidden className="h-5 w-5 shrink-0" /><span>{label}</span></button>)}</div></fieldset>
    <SpaceLoad {...summary} />
    {tab === "overview" && counts ? <>
      ...
      <section className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-extrabold">Resultado do time atual</h2><button type="button" onClick={() => { setResultsView("dashboard"); setTab("results"); }} className="text-sm font-bold text-blue-600">Ver detalhamento →</button></div><SpaceLoad {...results} />{results.data ? <TeamMetricCards data={results.data} compact /> : null}</section>
    </> : null}
    {tab === "pending" && summary.data ? <SpacePendingTab key={`${supervisorId}:${revision}:${pendingKind}`} initialKind={pendingKind} requiredCount={counts?.required ?? 0} supervisorId={supervisorId} lobs={summary.data.lobs} canRespond={summary.data.actor.canRespond} onAnswered={() => setAnsweredRevision((value) => value + 1)} /> : null}
    {tab === "results" ? <><SpaceLoad {...results} />{results.data ? <SpaceResultsTab key={supervisorId} data={results.data} supervisorId={supervisorId} view={resultsView} onViewChange={setResultsView} /> : null}</> : null}
    {tab === "hours" && summary.data ? <SpaceHoursTab key={`${supervisorId}:${revision}`} supervisorId={supervisorId} initialPeriod={period} /> : null}
    <p className="text-xs text-muted">As justificativas e os históricos permanecem nos fluxos originais. Requerido é apenas um alerta de cobertura, sem necessidade de justificativa.</p>
  </div>;
}
