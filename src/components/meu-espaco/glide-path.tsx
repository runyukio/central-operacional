  ...
  return <div className="space-y-4">
    ...
    {!read.loading && !read.error && data ? <>
      ...
      {!data.closed ? <details className="card space-y-3 p-4" aria-label="Simulação do volume restante"><summary className="cursor-pointer text-sm font-bold">Ver premissas ou ajustar cenário</summary>
        ...
        <p className="text-sm text-muted">Últimos 7 dias corridos até o corte: {dateLabel(data.recentStart)} a {dateLabel(data.cutoff)} · {data.recentDays} dias com base · {number(data.recentScheduled)} dias-parceiro com cronograma e dado válido. Cronograma após o corte: {number(data.futureScheduled)} dias-parceiro elegíveis. Parceiros sem base não entram como produtividade zero no ritmo observado.</p>
        ...
        <p className="text-xs text-muted">A simulação muda apenas este cenário. As metas, bases, cronogramas e horas permanecem iguais.</p>
      </details> : null}
