...
import { SpaceTargetCard } from "./target-card";
...
function metricColumns(lob: string, individual = false): MetricColumn[] {
  const cols: MetricColumn[] = lob === "CEC"
    ...
    : [{ key: "dailyIndividual", label: "Média diária individual", value: (m) => m.dailyIndividual }, { key: "ahtSeconds", label: "AHT", value: (m) => m.ahtSeconds, suffix: " s" },
      ...(!individual ? [{ key: "latencyMinutes", label: lob === "ADS" ? "Latência (h)" : "Latência vídeo (min)", value: (m: SpaceMetric) => m.latencyMinutes, latencyLob: lob }, { key: "commentsLatencyMinutes", label: "Latência Comments (h)", value: (m: SpaceMetric) => m.commentsLatencyMinutes, latencyLob: "COMMENTS" }] : [])];
...
function MetricCells({ metric, lob, individual = false }: { metric: SpaceMetric; lob: string; individual?: boolean }) {
  return <>{metricColumns(lob, individual).map((col) => <td key={col.key}>{col.latencyLob ? formatLatencyDisplay(col.value(metric), col.latencyLob) : number(col.value(metric), col.suffix)}</td>)}</>;
}
...
export function SpaceResultsTab({ data, supervisorId = "" }: { data: SpaceResults; supervisorId?: string }) {
  ...
  const navigation = <SpaceButtons label="Visão dos resultados" value={view} onChange={setView} options={[{ id: "dashboard", label: "Dashboard" }, { id: "glide", label: "Glide path" }]} />;
  if (view === "glide") return <div className="space-y-4">{navigation}<SpaceGlideTab key={supervisorId} supervisorId={supervisorId} lobs={data.groups.map((g) => g.lob).filter((lob) => ["ADS", "TNS", "CEC"].includes(lob))} /></div>;
