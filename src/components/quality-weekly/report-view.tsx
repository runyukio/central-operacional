...
import { change, number, rate, SECTION_NAMES } from "@/lib/quality-weekly/domain";
...
export function QualityReportView({ snapshot, initialSection = "CD", initialView = "queues" }: { snapshot: Snapshot; initialSection?: Section; initialView?: string }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2" aria-label="Report sections">{(Object.keys(SECTION_NAMES) as Section[]).map(key => <button type="button" key={key} aria-pressed={section === key} onClick={() => { setSection(key); setLimit(50); setSearch(""); setView("queues"); }} className={`${section === key ? "premium-button" : "premium-control text-navy-950"} px-4 py-2 text-sm font-extrabold`}>{SECTION_NAMES[key]} <span className="ml-2 opacity-70">{number(snapshot.sections[key].metrics.n)}</span></button>)}</div>
    <Panel title={SECTION_NAMES[section]}>
