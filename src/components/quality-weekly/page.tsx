...
import type { Issue, Mapping, Section, Snapshot, Validation } from "@/lib/quality-weekly/domain";
...
type Status = { user: { name: string }; mapping: { id: string; filename: string; entries: Mapping[]; createdBy: string; createdAt: string } | null };
...
export function QualityWeeklyPage({ accessError }: { accessError?: string }) {
  ...
  async function finishSelection(selected: Selection, selectedSheet: string, selectedDateColumn: string, signal: AbortSignal) {
    if (selected.kind === "mapping") {
      const result = await api<{ unchanged: boolean }>("mappings", { assetId: selected.assetId, sheet: selectedSheet }, signal);
      setStatus(await api<Status>("status", undefined, signal));
      setNotice(result.unchanged ? "This mapping is already active. No new mapping version was created." : "Queue mapping saved. Existing report versions were preserved. Revalidate any previous upload to use this mapping.");
    } else {
  ...
  const sourceMetrics = validation ? metrics(validation.sourceCounts) : null;
    ...
    {accessError ? null : <>
      ...
      {tab === "upload" && <>
        ...
        {validation && <Panel title="2. Validation & source reconciliation">
          <div className="mb-4 flex flex-wrap gap-3 text-sm font-bold"><span className={validation.valid ? "text-emerald-600" : "text-red-600"}>{validation.valid ? "Ready for weekly selection" : "Generation blocked"}</span><span className="text-muted">{validation.errorCount} errors · {validation.warningCount} warnings · {validation.duplicates} identical duplicates · {validation.summaryRows} summary rows excluded</span></div>
          <p className="mb-3 text-xs text-muted">Source row totals below are for reconciliation only, not an official weekly report. Validated weekly metrics use distinct case keys.</p>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-7">{[["Source cases", number(validation.sourceCounts.n)], ["Correct", number(validation.sourceCounts.correct)], ["Leakage", number(validation.sourceCounts.leakage)], ["False Positive", number(validation.sourceCounts.falsePositive)], ["Mislabeled", number(validation.sourceCounts.mislabeled)], ["Accuracy incl.", rate(sourceMetrics?.accuracy)], ["Accuracy excl.", rate(sourceMetrics?.adjustedAccuracy)]].map(([label, value]) => <div key={label} className="rounded-lg border border-border p-3"><p className="text-xs text-muted">{label}</p><p className="mt-1 text-lg font-extrabold text-navy-950">{value}</p></div>)}</div>
      ...
      {tab === "mapping" && <Panel title="Queue mapping · separate from operational Performance">
        <p className="text-sm text-muted">One classification per queue_id. Use section CD, ACCOUNTS or MATERIAL. ACCOUNTS requires Industry A or B; leave industry blank for other sections. Unknown or conflicting queues block generation.</p>
        ...
        {status?.mapping ? <><p className="my-4 text-xs text-muted">Active: {status.mapping.filename} · {dateTime(status.mapping.createdAt)} BRT · {status.mapping.createdBy}. Previous reports keep their original mapping.</p><div className="max-h-96 overflow-auto"><table className="w-full text-left text-xs text-navy-950"><thead className="bg-slate-50 text-muted"><tr>{["Queue ID", "Queue name", "Section", "Industry"].map(h => <th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{status.mapping.entries.map(m => <tr key={m.queueId} className="border-t border-border"><td className="p-3">{m.queueId}</td><td className="p-3">{m.queueName}</td><td className="p-3">{m.section}</td><td className="p-3">{m.industry || "—"}</td></tr>)}</tbody></table></div></> : <div className="mt-5"><EmptyState title="Queue mapping pending" description="Upload the approved de-para. No queue classification will be inferred from the reference document." /></div>}
      </Panel>}
