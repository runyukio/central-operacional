...
export function QualityWeeklyPage({ accessError }: { accessError?: string }) {
  ...
  async function finishSelection(selected: Selection, selectedSheet: string, selectedDateColumn: string, signal: AbortSignal) {
    if (selected.kind === "mapping") {
      ...
      setImported(result); setPreview(null); setSaved(null); setComplete(false); setReplace(false); setWeek(result.validation.dates.at(-1)?.start || ""); setWeekNumber(""); setTab("upload");
    }
    setSelection(null);
  }
    ...
    {accessError ? null : <>
      ...
      {tab === "upload" && <>
        <div className="grid items-start gap-4 lg:grid-cols-[1.25fr_1fr]">
          ...
          </ul><p className="mt-4 border-t border-border pt-3 text-xs text-muted">Monday–Friday periods are detected from moderation dates. Report blocks follow the mapping&apos;s section. The selected week and six previous periods are calculated from this upload; unavailable weeks remain absent. RCA is not part of this report.</p></Panel>
        </div>
        {validation && <Panel title="2. Validation & source reconciliation">
          ...
          <div className="mt-4 flex flex-wrap items-end gap-3"><label className="min-w-0 flex-1 text-xs font-bold text-muted">Moderation date column (only if a different header is used)<select aria-label="Moderation date column" className="premium-control mt-1 w-full px-3 py-2 text-sm" value={dateColumn} onChange={e => setDateColumn(e.target.value)}><option value="">Use recognized moderation date header</option>{validation.headers.filter(Boolean).map((h, i) => <option key={i} value={h}>{h}</option>)}</select></label><button type="button" disabled={Boolean(busy)} className="premium-control px-4 py-2 text-sm font-bold disabled:opacity-50" onClick={() => void run("Revalidating preserved source", async signal => {
            const result = await api<Imported>(`imports/${imported!.id}/revalidate`, { dateColumn: dateColumn || undefined }, signal); setImported(result); setPreview(null); setComplete(false); setWeek(result.validation.dates.at(-1)?.start || "");
        ...
        {validation?.valid && <Panel title="3. Select the operation's reporting week"><div className="grid gap-3 md:grid-cols-3">
          <label className="text-xs font-bold text-muted">Moderation week (Monday – Friday)<select aria-label="Moderation week" className="premium-control mt-1 w-full px-3 py-2 text-sm" value={week} onChange={e => { setWeek(e.target.value); setComplete(false); setPreview(null); }}><option value="">Select week</option>{[...validation.dates].reverse().map(d => <option key={d.start} value={d.start}>{d.start} → {d.end} ({number(d.count)} cases)</option>)}</select></label>
        ...
        </div><p className="mt-3 text-xs text-muted">{number(validation.weekendCases ?? 0)} weekend cases remain in the source but are excluded from weekly calculations. Observed dates indicate days with cases, not proof that an export is complete. Upload a broader range to populate earlier periods automatically.</p>
        <label className="mt-4 flex items-start gap-2 text-sm text-navy-950"><input type="checkbox" className="mt-1" checked={complete} onChange={e => setComplete(e.target.checked)} />I confirm the export is complete for {week || "the selected Monday"} to {week ? dayAdd(week, 4) : "Friday"} and for each historical period included, not a partial or filtered update.</label>
      ...
      {tab === "mapping" && <Panel title="Queue mapping · separate from operational Performance">
        <p className="text-sm text-muted">The section column defines report blocks and takes precedence over category. Material and Unit keep separate results. With no section, recognized categories are used; unclassified queues remain in Other queues and in the totals. Blank queue names are completed from the site registry when available. Only Accounts requires Industry A/B; conflicting queue IDs still block the upload.</p>
      ...
      {tab === "preview" && (snapshot ? <>
        ...
        {preview && !saved && <Panel title="Review before saving">
          {preview.previous ? <><p className="mb-3 text-sm text-muted">Compare with active version {preview.previous.version}. Saving creates a new version; the previous report remains available.</p><div className="overflow-x-auto"><table className="w-full text-left text-sm text-navy-950"><thead><tr><th className="p-2">Metric</th><th className="p-2">Active version</th><th className="p-2">New preview</th></tr></thead><tbody>{(["n", "correct", "leakage", "falsePositive", "mislabeled"] as const).map(key => <tr key={key} className="border-t border-border"><th className="p-2">{{ n: "Sampling Amount", correct: "Correct", leakage: "Leakage", falsePositive: "False Positive", mislabeled: "Mislabeled" }[key]}</th><td className="p-2">{number(preview.previous!.metrics[key])}</td><td className="p-2">{number(preview.snapshot.metrics[key])}</td></tr>)}</tbody></table></div><p className="mt-3 text-xs text-muted">Mapping version: {preview.previous.mappingId === snapshot.mappingId ? "Unchanged" : "Changed"}. Source digest: {preview.previous.digest === snapshot.digest ? "Unchanged" : "Changed"}. Week number: {preview.previous.weekNumber} → {snapshot.weekNumber}. Period end: {preview.previous.end} → {snapshot.end}. Rules: {preview.previous.ruleVersion} → {snapshot.ruleVersion}. Previous-week context: {JSON.stringify(preview.previous.trend.slice(0, -1)) === JSON.stringify(snapshot.trend.slice(0, -1)) ? "Unchanged" : "Changed"}.</p></> : <p className="text-sm text-muted">This is the first saved version for this moderation week.</p>}
