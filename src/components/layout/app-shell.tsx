...
export function AppShell({ children, user }: { children: React.ReactNode; user: ShellUser }) {
  ...
  return (
    <div className="min-h-screen bg-surface text-ink">
      <aside className="navy-gradient fixed inset-y-0 left-0 z-40 hidden w-[236px] border-r border-white/10 text-white shadow-2xl lg:flex lg:flex-col">
        <div className="flex h-[72px] items-center gap-3 border-b border-white/10 px-4">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-blue-500 to-blue-700 shadow-lg shadow-blue-950/40 ring-1 ring-white/20">
            <Sparkles className="h-5 w-5" />
          </div>
          <div>
            <p className="text-[16px] font-black leading-tight tracking-tight">Central</p>
            <p className="text-[16px] font-black leading-tight tracking-tight">Operacional</p>
          </div>
        </div>
        ...
        <nav className="sidebar-scroll flex-1 space-y-0.5 overflow-y-auto px-2.5 pb-4">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "group relative flex items-center gap-2.5 rounded-lg px-3 py-2 text-[12px] font-bold text-blue-50/86 transition",
                  active
        ...
        <div className="border-t border-white/10 p-3">
          <div className="rounded-xl border border-white/10 bg-white/8 p-3 shadow-lg shadow-navy-950/20">
            <p className="text-[13px] font-semibold">{isCollaborator ? "Precisa de ajuda?" : "Filtros por página"}</p>
      ...
      <main className="min-h-screen lg:pl-[236px]">
        <header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-border/80 bg-white/94 px-4 shadow-[0_8px_24px_rgba(7,27,58,0.035)] backdrop-blur-xl md:px-6">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <button className="premium-control grid h-10 w-10 place-items-center text-navy-900 lg:hidden">
              <Menu className="h-5 w-5" />
            </button>
            <div className="premium-control hidden h-10 w-full max-w-[460px] items-center gap-3 px-3 text-muted md:flex">
          ...
          <div className="flex items-center gap-2 text-navy-900">
            <div className="relative">
              ...
              {notificationOpen ? (
                <div className="absolute right-0 top-12 z-50 w-[340px] overflow-hidden rounded-2xl border border-border bg-white shadow-2xl shadow-navy-950/15">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative grid h-10 w-10 place-items-center rounded-full border border-border bg-gradient-to-b from-slate-50 to-slate-100 text-navy-900 shadow-soft">
        ...
        <div className="px-4 py-5 md:px-6 xl:px-7">{children}</div>
      </main>
    </div>
  );
}
