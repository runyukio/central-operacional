...
- Uploads de Escalas e Horas seguem o padrão de migração: uma requisição de preview por arquivo, uma requisição de commit por arquivo e processamento em chunks internos no backend. Escalas cruzam por `wb_login`, exigem `wb_login`, `data`, `status`, `turno`, `entrada`, `saida` e `lob`, validam colaboradores/turnos/LOBs em lote e gravam em blocos. Horas cruzam por `wb_login`, vinculam `Schedule` quando existir e gravam `WorkHourRecord` em blocos.
- Escalas Consolidadas possui download CSV em `/api/schedules/export`, respeitando mês/filtros aplicados e exportando `wb_login`, nome, e-mail, data, status, turno, entrada, saída, LOB, supervisor, observação e atualização.
- Cadastros agora pagina a esteira real com `page`, `limit`, `total` e `totalPages`; a tela mostra `Exibindo X-Y de Z`, permite 25/50/100 por página e navega além dos primeiros 100 registros importados.
